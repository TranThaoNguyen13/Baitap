...
public class InvoiceDao {
	        ...
	        if (connection != null) {
	            ...
	            if (rowsInserted > 0) {
	                ...
	                if (generatedKeys.next()) {
	                    ...
	                    String insertBillDetailQuery = "INSERT INTO BillDetail (InvoiceID, CosmeticID, Quantity, Price) VALUES (?, ?, ?, ?)";
	                    billDetailStatement = connection.prepareStatement(insertBillDetailQuery);
	                    for (BillDetail billDetail : billDetails) {
	                        billDetailStatement.setInt(1, invoiceID);
	                        billDetailStatement.setInt(2, billDetail.getCosmeticID());
	...
	public List<BillDetail> getAllBillDetailsByDate(Date fromDate, Date toDate) {
	    ...
	    try {
	        connection = SQLServerConnection.getConnection();
	        if (connection != null) {
	            String query = "SELECT bd.CosmeticID, bd.Quantity, bd.Price " +
	                           "FROM Invoice i " +
	            ...
	            resultSet = statement.executeQuery();
	            while (resultSet.next()) {
	                int cosmeticID = resultSet.getInt("CosmeticID");
	                int quantity = resultSet.getInt("Quantity");
	                float price = resultSet.getFloat("Price");

	                // Tạo đối tượng BillDetail từ dữ liệu truy vấn và thêm vào danh sách
	                BillDetail billDetail = new BillDetail(cosmeticID, quantity, price);
